...
@cli.command()
@click.option('-s', default="", prompt=u'-s 请指定一个或多个测试设备', help=u"设备id,支持多个设备同时，空格分割")
...
@click.option('--mode', type=click.Choice(['uiautomatormix', 'uiautomatordfs']), default="uiautomatormix", help=u"uiautomatormix是随机点击控件加monkey模式，uiautomatordfs是遍历控件模式")
@click.option('--runningminutes', default="1", help=u"执行时间")
@click.option('--throttle', default="700", help=u"点击间隔")
